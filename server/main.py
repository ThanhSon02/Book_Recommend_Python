...
def getHighestRatedBooks():
    ...
    books_df = books_df.dropna().drop(
        books_df[books_df["Year-Of-Publication"] == '0'].index)
    ratings_df = pd.read_csv(path + '\Ratings.csv')
    ratings_df = ratings_df.dropna().drop_duplicates(
        subset=['User-ID', 'ISBN'], keep='last')

    ratings_mean = ratings_df.groupby("ISBN").agg(
    ...
    highestRated = popular[popular["Rating-Count"] >
